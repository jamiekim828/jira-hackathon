...
import {Card,Collapse, CardHeader, CardContent, CardActions, IconButton, Typography, Box, TextField} from '@mui/material'
...
export default function ProjectDetail(){
    const project: ProjectType= useSelector((state: RootState)=> state.detailReducer.projectDetail)
    ...
    const temp = {id: id, name: name, creator: creator, description: description};
    ...
    >
      ...
      <TextField
        ...
        />
        <Button variant="contained" onClick={()=> {saveHandler(project)}}>Save</Button>
      </div>
      </Box>
      );
    }
                  ...
                  <CardContent>
                    <Typography>
                      {project.task.map((task)=>{
                        return <ul>
                              <li><Link to={`./task/${task.id}`}> {task.name}</Link></li>
                          </ul>
                      })}
                    </Typography>
                  </CardContent>
